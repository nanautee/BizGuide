...
const styles = {
  wrapper:
    "fixed inset-x-0 top-0 z-99 w-full max-w-[100vw] overflow-x-hidden px-3 pt-4 sm:px-6 sm:pt-6 md:px-10 lg:px-14",
  row: [
    "grid w-full min-w-0 max-w-full items-center gap-2 sm:gap-3",
    "grid-cols-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] sm:gap-4",
  ].join(" "),

  logo: [
    "z-99 min-w-0 shrink-0 justify-self-start text-lg font-bold leading-none text-white",
    "sm:text-3xl md:text-4xl lg:text-5xl",
  ].join(" "),

  pill: [
    "hidden rounded-full border border-white/35 bg-[#C7E5FF]/60 backdrop-blur-md",
    "sm:flex sm:h-[42px] sm:items-center sm:justify-center sm:justify-self-center sm:px-6",
    "md:h-[46px] md:px-8",
  ].join(" "),
  ...
  rightGroup: "z-10 flex min-w-0 shrink-0 items-center justify-end gap-1 justify-self-end sm:gap-2",

  headerBtn: [
    "flex h-8 max-w-full items-center gap-1 rounded-full",
    "border border-white/35 bg-white/20 px-2 backdrop-blur-sm",
    "transition hover:bg-white/30 active:scale-95",
    "sm:h-[42px] sm:gap-2 sm:px-4 md:h-[46px]",
  ].join(" "),
  headerBtnText: "text-xs font-normal text-white sm:text-sm md:text-base",
        ...
        <div className={styles.rightGroup}>
          {loggedIn ? (
            <>
              ...
              >
                <img src="/enter.svg" alt="" aria-hidden className={styles.headerBtnIcon} />
                <span className={styles.headerBtnText}>Личный кабинет</span>
              </Link>
