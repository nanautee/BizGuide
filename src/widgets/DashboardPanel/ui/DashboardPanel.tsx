...
const s = {
  ...
  metricCard: `${card} min-h-[100px] md:min-h-[130px]`,
  metricLabel: "text-sm text-[#4D84C0] md:text-base lg:text-lg",
  metricValue: [
    "mt-2 max-w-full whitespace-nowrap font-bold leading-none text-[#2E78C8]",
    "text-2xl sm:text-3xl md:text-[32px] lg:text-[38px]",
  ].join(" "),
  metricSub: "mt-2 text-xs leading-tight text-[#4D84C0] sm:text-sm md:text-base",
...
export function DashboardPanel({ initialData }: { initialData: DashboardInitialData }) {
  ...
  const socialBlocks = data.socialItems.map((item, i) => {
    ...
    const hasVkMetrics = !!m;
    const seed = hashSeed(`${item.url}:${item.platform}:${i}`);
    return {
      ...item,
      title: m?.name
        ? m.name
        : socialDrafts[i]?.name ?? `${platformLabel(item.platform as SocialLinkPayload["platform"])}. Группа ${i + 1}`,
      reach: seeded(seed + 1, 800, 3200),
      followers: hasVkMetrics ? m.membersCount : seeded(seed + 3, 12000, 260000),
      views: seeded(seed + 5, 500000, 2500000),
      posts: hasVkMetrics ? m.wallPosts : seeded(seed + 7, 6, 25),
      likes: seeded(seed + 9, 8000, 90000),
      verified: hasVkMetrics ? m.verified : false,
    ...
    <section className={s.page}>
      <div className={WRAP}>
        ...
        <div className={s.summaryRow}>
          ...
          <span className={s.summaryText}>
            Статус:{" "}
            <strong className={`font-bold ${statusColor[data.status] ?? "text-[#1A1A1A]"}`}>
              {data.status}
            </strong>
          </span>
